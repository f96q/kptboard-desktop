...
import LabelForm from '../lib/label_form'
import FluxStore from './flux_store'
...
class LabelStore extends FluxStore {
  ...
  invokeOnDispatch(action) {
    switch (action.type) {
      case LabelConstants.LABEL_SET_LABELS:
        Object.assign(this.state.labels, action.labels);
        this.emitChange();
      break;

      case LabelConstants.LABEL_OPEN_DIALOG: {
        let label = null;
        if (action.label.id) {
      ...
      case LabelConstants.LABEL_CREATE:
        this.create(action.label);
        this.emitChange();
      break;

      case LabelConstants.LABEL_UPDATE:
        this.update(action.id, action.label);
        this.emitChange();
      break;

      case LabelConstants.LABEL_DESTROY:
        this.destroy(action.id);
        this.emitChange();
      break;

      case LabelConstants.LABEL_DRAG_START:
        this.state.dragStartId = action.id;
        this.emitChange();
      break;

      case LabelConstants.LABEL_DRAG_END:
        this.state.dragStartId = null;
        this.emitChange();
      break;

      case LabelConstants.LABEL_DROP:
...
export default new LabelStore(Dispatcher);
